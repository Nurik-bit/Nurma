...
class TestAppDatabaseIntegration(unittest.TestCase):
    def setUp(self):
        ...
        self.db_path = os.path.join(os.path.dirname(__file__), 'database', 'translations.db')
    ...
    def test_post_translation_and_save_to_db(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM translations WHERE source_text = 'Hello' AND translated_text = 'Привет'")
    ...
    def test_delete_translation(self):
        ...
        response = self.app.get(f'/delete_history/{translation_id}', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
