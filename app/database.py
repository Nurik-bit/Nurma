...
# Путь к базе данных
DATABASE_PATH = os.path.join(os.path.dirname(__file__), '..', 'database', 'translations.db')
...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS translation_history (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def log_and_report_translation(source_text, translated_text, source_language, target_language, translation_date):
    """Сохраняет лог и добавляет запись в HTML-отчет."""
    log_entry = (f"[{translation_date}] Перевод: '{source_text}' ({source_language}) → "
...
def save_translation(source_text, translated_text, source_language, target_language, translation_date):
    """Сохраняем перевод в таблицы translations и translation_history."""
    conn = create_connection()
    cursor = conn.cursor()

    # Вставляем данные в таблицу translations
    cursor.execute("INSERT INTO translations (source_text, translated_text) VALUES (?, ?)", 
    ...
    cursor.execute("INSERT INTO translation_history (translation_id, source_language, target_language, translation_date) VALUES (?, ?, ?, ?)", 
    ...
    log_and_report_translation(source_text, translated_text, source_language, target_language, translation_date)
...
def get_translation_history():
    ...
    cursor.execute('''SELECT th.id, t.source_text, t.translated_text, th.source_language, th.target_language, th.translation_date
...
def delete_translation(translation_id):
    try:
        ...
        print(f"Attempting to delete record with ID: {translation_id}")  # Выводим для проверки

        c.execute('DELETE FROM translation_history WHERE translation_id = ?', (translation_id,))
        c.execute('DELETE FROM translations WHERE id = ?', (translation_id,))  # Удаляем из обеих таблиц
